...
\caption{A $K_s$-band image of $\omega$~Cen from the FourStar camera, overlaid with a catalog of RRL from \citet{2004A&A...424.1101K} and footprints of the {\it Spitzer} IRAC fields. \textcolor{magenta}{The} circular points are RRab's and the triangular points are RRc's; we adopt this convention throughout the paper. The three black rectangle outlines are the IRAC field of view for each pointing in the 3.6~$\mu$m channel, and the white rectangle outlines \textcolor{magenta}{show} the same for 4.5~$\mu$m.} 
...
PSF photometry was performed using {\sc daophot} and {\sc allframe} \citep{1987PASP...99..191S, 1994PASP..106..250S}. The PSF model was created for each field/filter combination using the first epoch data \textcolor{magenta}{and was applied to every} epoch. As the observations were taken temporally close together the effects of telescope rotation between epochs on the mosaicked PSF were minimal, so making a single good PSF model for each field/filter combination was much more efficient than creating one for every epoch. 
...
\begin{table}
\centering
\caption{Empirical mid-IR RRL period-luminosity relation coefficients for $\omega$~Cen \citep{2015ApJ...808...11N}, for relations of the form $M = a + b \times (\log (P) + P_0)$ with intrinsic dispersion $\sigma$.\vscomment{This caption needs to be clarified. These are actually the PL relations from Neeley et al, assuming an M4 distance of 11.399. They don't assume anything about $\omega$~Cen. Having $\omega$~Cen in the caption confuses what they actually are.}} 
...
\begin{table}
\centering
\caption{Theoretical near-IR RRL period-luminosity relation coefficients for $\omega$ Cen \citep{2015ApJ...808...50M}, for relations of the form $M = a + b \times \log P + c \times [\text{Fe/H}]$ with intrinsic dispersion $\sigma$. \vscomment{Same as previous caption. These parameters are not derived for/from $\omega$~Cen, so remove the reference to it. Also only use 2 decimal places for this table as that's how many are used in the original paper.}}
...
It is apparent from Figure~\ref{fig:omegaCen_dist_m4_mean} that there are large discrepancies in the distance moduli in [3.6] and [4.5] for the two pulsation modes; these \textcolor{magenta}{discrepancies} contribute to the relatively low $E(B-V)$ value and high dereddened distance modulus \textcolor{magenta}{compared to previously determined values} \citep[e.g.][]{2002ASPC..265...95L, 2006ApJ...652..362D}. 
...
$\omega$ Cen is ideal for examining the RRL period-luminosity-metallicity relation, because \textcolor{magenta}{ it is known to have a large spread in metallicity \citep[$0.8~\leq~\Delta~\text{[Fe/H]}~\leq~1.4$~dex][]{2014ApJ...791..107V, 2012ApJ...746...14M, 2010ApJ...722.1373J}}.
...
We know from mid-IR spectra that a significant CO feature sits within the IRAC [4.5] filter. In the case of Cepheids, \vscomment{Cite Scowcroft et al. 2016, MNRAS in press, will give you arxiv number on Monday} have shown that this has a significant effect on the [4.5] magnitudes, and is metallicity dependent. However, this effect decreases with increasing temperatures, turning off completely above 6000~K where all the CO has been destroyed \textcolor{magenta}{(Scowcroft et al. 2016)}. As even the coolest RRL have temperatures over 6000~K \citep{1971PASP...83..697I}, we expect to see no such CO absorption in the [4.5] PL relation. However, we can directly and empirically test this prediction. %If there are any other unanticipated metallicity effects for RRL, they must be smaller than the dispersion of the PL relations themselves, but we must still perform empirical tests to search for such effects.

\vscomment{So I'm not sure about this paragraph. The best way to directly test for the CO thing is a colour curve, and we aren't doing that. I've done that in another paper (in prep), and they're flat. I think maybe we want to change the emphasis of this paragraph to be about directly and empirically testing for {\bf any/all} metallicity effects, rather than just the CO. I think it would just take an extra sentance or two to extend it. Try it and see what you think.}.
