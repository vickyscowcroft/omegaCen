...
We also anticipate that the NIRCam instrument on {\em JWST} \citep{2005SPIE.5904...21B, 2006SSRv..123..485G} will provide substantial improvements over IRAC for this project. The NIRCam filters F356W and F444W will provide data in passbands comparable to IRAC's [3.6] and [4.5] at an order of magnitude higher resolution (0.065 arcsec/pixel), which will significantly decrease photometric error due to crowding and therefore increase the sample size of RRLs. % lol unless it blows up on launch/gets stuck unfolding/fails to align its mirrors/crashes into everything else that's at L2

% Eric Persson's metallicity omega cen project - 'we explore a subset of RR Lyrae variables in the ? Centauri globular cluster for which a large set of stellar metallicities will be available for a broad range of RR Lyrae' overview paper
