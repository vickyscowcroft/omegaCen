...
\caption{A $K_s$--band image of $\omega$~Cen from the FourStar camera, overlaid with the catalog of RR Lyrae \citep{2004A&A...424.1101K} and footprints of the {\it Spitzer} IRAC fields.}
...
{\bf Andy to add text about FourStar data and reduction}

\subsection{Crowding}
\label{sec:crowding}

The primary limiting factor in this data is crowding: 77 RR Lyrae variables out of the original catalog of 192 \citep{2004A&A...424.1101K} were rejected due to crowding. We compared the {\it Spitzer} images to a $K_s$--band image from the FourStar infrared camera on Magellan \citep{2013PASP..125..654P}, with a resolution of 0.159 arcsec. This enabled us to see which stars were significantly contaminated. Our full, uncrowded RR Lyrae sample consists of 96 stars in $J$ and $H$, 98 in $K_s$, 36 in 3.6~$\mu$m, and 43 in 4.5~$\mu$m.
...
Our full, uncrowded RR Lyrae sample consists of 96 stars in $J$ and $H$, 98 in $K_s$, 36 in 3.6~$\mu$m, and 43 in 4.5~$\mu$m. The PL fits to this sample are shown in Figure~\ref{fig:omegaCen_pl_allstars}.
...
%% Then you can cut it down, once you've explained what you're cutting from:
...
%% It's not clear from the caption whether the PL relations here are from the theoretical or empirical relations. You need to be very explicit about which fits are coming from which. 
...
%and the distance moduli and reddening values obtained from these fits are shown in Figure~\ref{fig:omegaCen_dist_phot_allstar}.
...
\caption{Distance moduli for the cut sample of $J\!H\!K_s$, 3.6~$\mu$m, and 4.5~$\mu$m photometry using the average photometric metallicity from \citet{2000AJ....119.1824R} and theoretical PL relations for all bandpasses}
...
\caption{Distance moduli for the cut sample of $J\!H\!K_s$, 3.6~$\mu$m, and 4.5~$\mu$m photometry using the average spectroscopic metallicity from \citet{2006ApJ...640L..43S} and theoretical PL relations for all bandpasses}
...
\begin{figure}
\begin{center}
\includegraphics[width=80mm]{final_plots/multiwavelength_distance_samestars_m4_no4p5c.pdf}
\caption{Distance moduli for the cut sample of $J\!H\!K_s$, 3.6~$\mu$m, and 4.5~$\mu$m photometry using the average photometric metallicity from \citet{2006ApJ...640L..43S} and theoretical PL relations for $J\!H\!K_s$ and empirical PL relations for 3.6~$\mu$m and 4.5~$\mu$m, excluding the first overtone pulsators in 4.5~$\mu$m}
...
\caption{Period color nonsense} %, using the average photometric metallicity from \citet{2006ApJ...640L..43S} and theoretical PL relations for $J\!H\!K_s$ and empirical PL relations for 3.6~$\mu$m and 4.5~$\mu$m, excluding the first overtone pulsators in 4.5~$\mu$m}
...
\section{Appendix: Photometry of RR Lyrae Variables}
...
\caption{$J\!H\!K_s$, 3.6~$\mu$m, and 4.5~$\mu$m photometry of the RR Lyrae variables in $\omega$~Cen\label{tab:phot}} %% Your numbering problem can be solved by putting the label inside the caption. Labels in tex correspond to the captions, not the objects. 
\tabularnewline 
ID & RA (J2000) & Dec (J2000)  & $J$  & $\sigma_{J}$ & $H$   & $\sigma_{H}$   & $K_s$   & $\sigma_{K}$ & [3.6]   & $\sigma_{{[3.6]}}$ & [4.5]  & $\sigma_{{[4.5]}}$  & $P$ (days) & Mode & [Fe/H], p   & $\sigma_{[\text{Fe/H}]}$, p   & [Fe/H], s   & $\sigma_{[\text{Fe/H}]}$, s \\
\hline
\endfirsthead
\multicolumn{4}{c}%
...
\hline
\endlastfoot
3 & 13:25:56.15 & -47:25:53.8 & 13.247 & 0.017 & 12.982 & 0.018 & 12.882 & 0.017 & 12.841 & 0.039 & 12.708 & 0.036 & 0.841 & ab & -1.54 & 0.05 & -- & -- \\
