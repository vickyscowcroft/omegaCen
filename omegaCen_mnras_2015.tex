...
\title[Mid-IR RRL PL Relation in $\omega$ Cen]{The Carnegie RR Lyrae Program: The Mid-Infrared RR Lyrae Period-Luminosity Relation in $\omega$~Cen and also metallicity}
...
The primary limiting factor in the data is crowding: 77 RRLs out of the original catalog of 192 \citep{2004A&A...424.1101K} were rejected due to crowding. To assess crowding, we compared the {\it Spitzer} images to the FourStar $K_s$-band image. The 0.159 arcsec/pixel resolution of the $K_s$ band image compared to the 0.6 arcsec/pixel resolution of the IRAC images enabled us to more accurately determine which stars were significantly contaminated. %Our full, uncrowded RRL sample consists of 97 stars in $J$ and $H$, 99 in $K_s$, 37 in 3.6~$\mu$m, and 43 in 4.5~$\mu$m. {\bf [CHECK THIS]}

\section{Results}
\label{sec:results}

Our final photometry catalog, including magnitudes and uncertainties for $J\!H\!K_s$, [3.6], and [4.5], is presented in Table~\ref{tab:phot}. {\bf Note: this will not be an appendix in the final draft, we're just putting it there for now because it's huge.}
%% See comments in appendix for how I got the appendix table numbering to work properly.
The average magnitudes presented in Table~\ref{tab:phot} are flux averages, and the photometric uncertainties of the time series data are the error on the mean. Gloess fitting [citation??] was performed on the light curves as done in [other CCHP papers], but it was not found to improve results.
...
\begin{table}
\centering
\caption{Empirical mid-IR RRL period-luminosity relation coefficients for $\omega$~Cen \citep{2015arXiv150507858N}, for relations of the form $M = a + b \times (\log (P) + P_0)$ with intrinsic dispersion $\sigma$.} 
...
The distance moduli derived from the PL fitting have not been corrected for reddening. {\bf More stuff about this here}

We combine the uncorrected distance moduli from each bandpass to obtain a mean reddening-corrected distance modulus. We fit the near-infrared reddening law from \citet{1989ApJ...345..245C} to the $JHK_s$ data and the mid-infrared law from \citet{2005ApJ...619..931I} to [3.6] and [4.5] simultaneously, assuming a ratio of total to selective absorption $R_V = 3.1$. The resulting fit is shown in Figure~\ref{fig:omegaCen_dist_m4_mean}. We derive a dereddened distance modulus of $\langle \mu_0 \rangle = 13.781 \pm 0.018$ with $E(B-V) = 0.066 \pm 0.030$ using the weighted mean RRab + RRc distances. The individual uncorrected distance moduli $\mu$, corrected distance moduli $\mu_0$, and PL residuals are shown in Table~\ref{tab:dist_mod}.
...
\hline
$[3.6]$ & RRab & 0.088 & 0.035 & 0.048 & $0.064$ \\ % & $0.282$ & $0.234$\\
$[3.6]$ & RRc & 0.102 & 0.021 & 0.041 & $0.091$ \\ %& $0.778$ & $0.323$\\
$[4.5]$ & RRab & 0.173 & 0.036 & 0.046 & $0.162$ \\ % & $0.712$ & $0.590$\\
$[4.5]$ & RRc & 0.202 & 0.021 & 0.048 & $0.195$ \\ %& $1.660$ & $0.689$\\
\hline
\end{tabular}
\end{table}
...
\caption{Top: Intrinsic, photometric error, and observed PL relations as in Figure~\ref{fig:probdist1}, as well as the scaled metallicity distribution and best model (convolution of intrinsic, photometric, and metallicity distributions). Bottom: only the observed and model distributions for easier comparison.}
\label{fig:probdist2}
\end{center}
\end{figure}
...
\end{equation}
to the [3.6] and [4.5] PL residuals and metallicity values for stars with known individual metallicity values, as shown in Figure~\ref{fig:metallicity_residuals}. By this metric we find that although the scatter in the 3.6~$\mu$m and 4.5~$\mu$m PL relations is higher for $\omega$~Cen than it is for M4 \citep{2015arXiv150507858N, 2015ApJ...799..165B}, there is currently not enough evidence to ascribe it to metallicity alone. When we examine [Fe/H] vs. the residuals of each PL relation, $\gamma$ is within $1\sigma$ of zero for all fits, indicating that there is no significant metallicity dependence in the PL residuals.

\begin{figure*}
\begin{center}
\includegraphics[width=160mm]{reworked_fitting_code/final_plots/metallicity_vs_residuals_m4_split.pdf}
\caption{Photometric \citep{2000AJ....119.1824R} and spectroscopic \citep{2006ApJ...640L..43S} [Fe/H] values vs. period-luminosity residuals for RRab and RRc in [3.6] and [4.5], with the $\gamma$ parameter from equation 4 in the top right corner of each subplot.}
...
We derive a true mean dereddened distance modulus to $\omega$~Cen of $\langle \mu_0 \rangle = 13.781 \pm 0.018$ with reddening $E(B-V) = 0.066 \pm 0.030$ using mean distance moduli of RRab and RRc PL relations in $JHK_s$, [3.6], and [4.6]. Using only RRab in the same passbands, we derive a true mean dereddened distance modulus of $\langle \mu_0 \rangle = 13.739 \pm 0.024$ with reddening $E(B-V) = 0.110 \pm 0.042$. The RRab results are closer to literature values.
...
% Claremont-Carnegie Astrophysics Research Program - poke Phil
...
\caption{NEEDS MORE DESCRIPTION $J\!H\!K_s$, 3.6~$\mu$m, and 4.5~$\mu$m photometry of the RRLs in $\omega$~Cen\label{tab:phot}} 
\tabularnewline 
