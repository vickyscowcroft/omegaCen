...
{\bf This section will change--will discuss at meeting}
...
We can place an upper limit on the contribution of metallicity to the measured PL dispersion by combining the probability distributions of the known intrinsic PL width and the scatter induced by photometric error and comparing the result to the observed distribution of $\omega$~Cen PL residuals. We model the intrinsic scatter as a uniform distribution with full width $\sqrt{12} \times 0.035 = 0.121$ and the photometric uncertainties as a Gaussian with a standard deviation of the median photometric error in the sample, $\sigma = 0.046$. We construct a probability distribution for the observed PL scatter using a Gaussian kernel density estimator on the residuals with a bandwidth selected by leave-one-out cross-validation \citep{2013sdmm.book.....I}. All these models are shown in Figure~\ref{fig:probdist1}, along with the convolution of the intrinsic scatter and photometric error distributions.

We construct a metallicity probability distribution in the same way as for the observed scatter. We show the distributions overlaid on histograms of the known metallicity values in Figure~\ref{fig:feh_hist} for all metallicity values and for only the ones that appear in our sample of RRab in [3.6]. Our RRab sample may oversample the high-metallicity end of the distribution, but as we do not have metallicity values for all the stars the sample, we use the distribution for all metallicity values.

Figure~\ref{fig:probdist2}
...
\caption{Some probability densities.}
\label{fig:probdist2}
\end{center}
\end{figure}
